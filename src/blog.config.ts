const avatarUrl = "https://github.com/hsn8086.png";
const userName = "HSN8086";
const bio = "Time Limit Exceeded.";

const tags = ["ACMer", "Neko", "Python", "FastAPI", "Vue 3", "Arch Linux"];
/*

links:
  - name: 咲夜の小屋
    url: https://sakuya.moe/
    image: https://bu.dusays.com/2023/11/28/6565a69d55ea4.png
    desc: 我可真是个大聪明~！
  - name: L4plAce
    url: https://www.l4place.icu/
    image: https://www.l4place.icu/img/avatar.jpg
    desc: 以空想为罗盘，以实践作航船
  - name: SamuFlore's Dimension
    url: https://samuflore.top/
    image: https://samuflore.top/images/avatar.png
    desc: 德才兼備、知行合一
  - name: 雪花の博客
    url: https://xhblog.top/
    avatar: https://xhblog.top/img/724d7fb480c8ac0db472ca5c7e36d239.jpg
    desc: =爬起仅仅只是因为不想输。
  - name: 御坂晚的笨蛋笔记
    url: https://starneko.com/
    avatar: https://starneko.com/img/Mahiro.jpg
    desc: 这里什么都没有呢
  - name: 薄荷の小屋
    url: https://www.hoshiroko.com/
    avatar: https://api.hoshiroko.com/img/avatar.jpg
    desc: 越是拼命往前伸手，渴望之物越是渐行渐远
  - name: 猪猪博客
    url: https://www.zzwl.top/
    avatar: https://www.zzwl.top/favicon.ico
    desc: 没有描述
  - name: Farewe1ll's Blog
    url: https://blog.farewe1ll.com
    avatar: https://avatar.farewe1ll.top
    desc: 撥雪尋春，燒燈續晝。
    */
const links = [
  {
    name: "咲夜の小屋",
    url: "https://sakuya.moe/",
    avatar: "https://bu.dusays.com/2023/11/28/6565a69d55ea4.png",
    desc: "我可真是个大聪明~！",
  },
  {
    name: "L4plAce",
    url: "https://www.l4place.icu/",
    avatar: "https://www.l4place.icu/img/avatar.jpg",
    desc: "以空想为罗盘，以实践作航船",
  },
  {
    name: "SamuFlore's Dimension",
    url: "https://samuflore.top/",
    avatar: "https://samuflore.top/images/avatar.png",
    desc: "德才兼備、知行合一",
  },
  {
    name: "雪花の博客",
    url: "https://xhblog.top/",
    avatar: "https://xhblog.top/img/724d7fb480c8ac0db472ca5c7e36d239.jpg",
    desc: "=爬起仅仅只是因为不想输。",
  },
  {
    name: "御坂晚的笨蛋笔记",
    url: "https://starneko.com/",
    avatar: "https://starneko.com/img/Mahiro.jpg",
    desc: "这里什么都没有呢",
  },
  {
    name: "薄荷の小屋",
    url: "https://www.hoshiroko.com/",
    avatar: "https://api.hoshiroko.com/img/avatar.jpg",
    desc: "越是拼命往前伸手，渴望之物越是渐行渐远",
  },
  {
    name: "猪猪博客",
    url: "https://www.zzwl.top/",
    avatar: "https://www.zzwl.top/favicon.ico",
    desc: "没有描述",
  },
  {
    name: "Farewe1ll's Blog",
    url: "https://blog.farewe1ll.com",
    avatar: "https://avatar.farewe1ll.top",
    desc: "撥雪尋春，燒燈續晝。",
  },
];
export { userName, bio, avatarUrl, tags, links };
